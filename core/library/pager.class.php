        ...
        if (isset($data['prepage'])) {
            foreach ($data['prepage'] as $lines) {
                if ($lines['current'] === true) {
                    $html .= '<span class="pwidth">' . $lines['text'] . '</span >';
                } else {
                    $content = ($data['ajax'] === true) ? "<a class='pwidth' href='{$lines['url']}' onclick='{$data['ajaxaction']}('{$lines['url']}'); return false;'>{$lines['text']}</a>" : "<a class='pwidth' href='{$lines['url']}' target='_self'>{$lines['text']}</a>";
                    $html .= $content;
                }
            }
        }
        ...
        if (isset($data['nextpage'])) {
            foreach ($data['nextpage'] as $lines) {
                if ($lines['current'] === true) {
                    ...
                    $content = ($data['ajax'] === true) ? "<a class='pwidth' href='{$lines['url']}' onclick='{$data['ajaxaction']}('{$lines['url']}'); return false;'>{$lines['text']}</a>" : "<a class='pwidth' href='{$lines['url']}' >{$lines['text']}</a>";
                    $html .= $content;
                }
            }
        }
    ...
    protected function _processData()
    {
        ...
        if($this->_page==1){
            $data['prepage'] = array(
                ...
                array('text' => $this->prePage, 'url' => str_replace('[page]', ($this->_page - 1), $this->_url),'current' => true),
            );
        }else{
            $data['prepage'] = array(
                array('text' => $this->firstPage, 'url' => str_replace('[page]', 1, $this->_url),'current'=>false),
        ...
        if($this->_page==$this->_totalPages){
            $data['nextpage'] = array(
                array('text' => $this->nextPage, 'url' => str_replace('[page]', ($this->_page + 1), $this->_url),'current'=>true),
                array('text' => $this->lastPage, 'url' => str_replace('[page]', $this->_totalPages, $this->_url)),
            );
        }else{
            $data['nextpage'] = array(
                array('text' => $this->nextPage, 'url' => str_replace('[page]', ($this->_page + 1), $this->_url),'current'=>false),
                array('text' => $this->lastPage, 'url' => str_replace('[page]', $this->_totalPages, $this->_url)),
            );
        }
